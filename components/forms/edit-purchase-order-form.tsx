...
import { useUpdatePurchaseOrder } from '@/lib/hooks/usePurchaseOrders';
...
import type { PurchaseOrderResponse } from '@/types/purchase-orders.types';

/**
 * Edit Purchase Order Form Component
 *
 * C# equivalent: Edit view with form bound to existing PO entity
 */
...
export function EditPurchaseOrderForm({
   ...
   *
   * Important: We map the items array to include purchaseOrderItemId
   * This tells the backend which items to UPDATE vs CREATE
   */
  const form = useForm<PurchaseOrderFormData>({
    ...
    defaultValues: {
      ...
      items: initialData.items.map((item) => ({
        purchaseOrderItemId: item.purchaseOrderItemId, // ← Existing item ID
   ...
   * (e.g., user navigates to different edit page)
   */
  useEffect(() => {
    form.reset({
      projectId: initialData.projectId,
  ...
  /**
   * Add new line item
   * New items don't have purchaseOrderItemId
   */
  const handleAddItem = () => {
    append({
      // No purchaseOrderItemId → backend will CREATE this item
      materialId: 0,
      quantity: 0,
      unitPrice: 0,
    });
   ...
   * Remove line item
   *
   * If item has purchaseOrderItemId and is removed from array,
   * backend will DELETE it from database
   */
   ...
   * C# equivalent:
   * [HttpPut("{id}")]
   * public async Task<IActionResult> Update(int id, UpdatePurchaseOrderRequest request)
   */
                  ...
                  {fields.map((field, index) => {
                    ...
                    return (
                      <TableRow key={field.id}>
                        {/* Hidden field to preserve purchaseOrderItemId */}
                        <FormField
                          control={form.control}
